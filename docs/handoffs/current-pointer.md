...
- `pending local checkpoint` — every non-empty kanban lane now uses the same hover/focus preview tray behavior, with lane-aware placement so left, middle, and right columns expand in a natural direction without throwing previews off-screen
