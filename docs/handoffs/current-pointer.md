...
- `5366e97` — added an isolated `/dashboard-stretch-lab` playground route on the `stretch-lab-experiment` branch; it duplicates the cockpit and replaces floating preview trays with true spring-driven lane stretching/compression so the interaction can be judged without touching the real cockpit route
...
- Dashboard / cockpit `NEW` is now a personalized Kansas City discovery pool tuned to the current user’s background instead of a freshness-only 50-card slice.
  ...
  - Hovering or focusing any non-empty lane now opens a wider preview tray above the board so multiple jobs can be compared with fuller summaries before opening a workspace.
- Kanban readability was improved.
  ...
  - Decorative per-card icons were removed so the cards read as clean text-first summaries.
...
- Verification:
  ...
  - browser-verified on `/dashboard-wireframe`:
    ...
    - the lanes stay fixed-height and can be grabbed to browse the full match pool without moving the workspace
    - `INTERESTED` now opens the same richer hover tray as `NEW`
- New proof artifacts:
  ...
  - `/home/richard/code/jobs/job-search-platform/output/playwright/dashboard-wireframe-all-lanes-preview.png`
