                    ...
                    {stats.map((stat, index) => {
                        ...
                        return (
                            <ShellCard
                                key={stat.label}
                                glowColor={stat.glow}
                                className="min-h-[160px] relative overflow-hidden group"
                                style={{
                                    background: `radial-gradient(circle at top right, ${stat.glow.replace('0.5', '0.15')}, transparent 60%)`,
                                }}
                            >
                                <div className="flex flex-col justify-between h-full space-y-3 relative z-10">
                                    <div className="flex justify-between items-start">
                                        {/* Icon with subtle radial glow - now smaller and more refined */}
                                        <div className="relative">
                                            <div
                                                className="absolute inset-0 blur-lg opacity-20 rounded-full scale-125"
                                        ...
                                        <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-white/5 text-muted-foreground border border-white/5 uppercase tracking-wider">
                                            {stat.change}
                                        </span>
                                    </div>

                                    <div className="space-y-1">
                                        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em] opacity-70">{stat.label}</p>
                                        <div className={`text-4xl font-bold tracking-tighter ${stat.color} leading-none`}>
            ...
            <div className="grid gap-6 md:grid-cols-3 lg:grid-cols-7">
                ...
                <div className="md:col-span-2 lg:col-span-4">
                    <ShellCard
                        ...
                        action={
                            <Button variant="ghost" size="sm" className="h-8 text-[10px] text-muted-foreground hover:text-primary uppercase tracking-widest">View All</Button>
                        }
                    ...
                    >
                        ...
                        <div className="relative z-10 space-y-6 flex flex-col items-center">
                            ...
                            <Link href={"/jobs" as Route}>
                                <Button size="sm" className="rounded-full px-6 h-10 text-xs shadow-lg shadow-primary/25 hover:shadow-primary/50 transition-all">
                                    Browse Jobs
                                </Button>
                            </Link>
                        </div>
