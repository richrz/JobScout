...
- **Absolute Compliance**: Strictly adhere to `TDD-AGENTS.md`. Any deviation is a critical failure.
...
1. **Mandatory First Step**: **READ `TDD-AGENTS.md` WORD-FOR-WORD**. Do not skim. This file contains the immutable laws of this environment.
2. **Pre‑Session Setup** – After reading `TDD-AGENTS.md`, read guardrails, verify a clean repository, and ensure the task list is loaded.
2. **Task Claiming** – Run `./scripts/work-start.sh` to lock the task.
...
```json
{
  ...
  "operatingProcedures": [
    ...
    "Task Claiming: Use ./scripts/work-start.sh to begin session properly",
